...
DataCells = [{'Run'},{'Well'},{'# Cells'},{'Gal8 sum'},{'Gal8/cell'},...
    {'# Foci'},{'# Foci/cell'},{'Cy5 sum'},{'# NPs'},{'# NPs/cell'},{'#NPs overlap Gal8'}]; 
...
for j=1:length(listing) % all images
    ...
    % Overlap study
    basinmap4=watershed(~(gal8pos2 & uptakepos2));
    xarea=gal8pos2 & uptakepos2;
    xmap=basinmap4;
    xmap(~xarea)=0;
    numoverlap = max(xmap(:));
    ...
    DataCells=[DataCells;{run},{well},{numcell},{galsum},{uint64(galsum)...
        ...
        {cy5int},{numnp},{double(numnp)/double(numcell)},{numoverlap}]
