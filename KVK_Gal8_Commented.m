...
for j=1:length(listing) % all images
    ...
    gal8pos1=gal8th>galectin8_threshold;
    gal8pos2=imopen(gal8pos1,se3);
    ...
    nuc=(imopen(nthr,se10));
    basinmap=watershed(~nuc);
    cellarea=imdilate(nuc, se3) ;
    cellmap=basinmap; cellmap(~cellarea)=0;
    nucmap=(label2rgb(cellmap)); %Generates "sanity check" rainbow map
...
writetable(cell2table(DataCells),strcat(exportbase,'.xls')); % Exports an csv sheet of your data