...
for i = 1:numImages % Iterate over all images.
    ...
    nuc = series1{1, 1}; % Nuclei image is the second channel.
    uptake = series1{2, 1}; % Cy5 (or DiD) image is the third channel.
...
%     gal8Sum = sum(gal8Thresh, 'all');
    ...
    NPResults = identifyFoci(NPTH, NP_THRESHOLD, SE_NP_OP);
    numNPs = NPResults{1};
...
%     end
    if EXPORT_COMPOSITE
        ...
        comp = cat(2, NPThresh.*NP_BRIGHTEN,...
            nucThresh.*NUC_BRIGHTEN);
