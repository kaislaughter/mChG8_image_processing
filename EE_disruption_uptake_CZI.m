...
optionalKeys = {...
    ...
    'EXPORT_NP_MAP',...
    'EXPORT_PL_MAP',...
    'EXPORT_GAL8_ANNOTATIONS',...
    'EXPORT_NP_ANNOTATIONS',...
    'EXPORT_PL_ANNOTATIONS',...
    'EXPORT_OVERLAP_ANNOTATIONS',...
    'EXPORT_UNFILTERED_COMPOSITE',...
    'EXPORT_COMPOSITE',...
    'EXPORT_CORRELATION_PLOTS',...
...
for i = 1:numImages % Iterate over all images.
    ...
    if config('EXPORT_GAL8_MAP')
        % Generate and save "sanity check" rainbow map for Gal8 foci.
        disp('Exporting Gal8 map...');
        gal8Map = label2rgb(gal8Labels, 'jet', 'w', 'shuffle');
        imwrite(gal8Map, strcat(exportbase, 'gal8map', '.png'),...
            config('FILETYPE'));
    end
    if config('EXPORT_NP_MAP')
        % Generate and save "sanity check" rainbow map for NPs.
        disp('Exporting NP map...');
        NPMap = label2rgb(NPLabels, 'jet', 'w', 'shuffle');
        imwrite(NPMap, strcat(exportbase, 'npmap', '.png'),...
            config('FILETYPE'));
    end
    if config('EXPORT_PL_MAP')
        % Generate and save "sanity check" rainbow map for PL vesicles.
        disp('Exporting PL map...');
        PLMap = label2rgb(PLLabels, 'jet', 'w', 'shuffle');
        imwrite(PLMap, strcat(exportbase, 'plmap', '.png'),...
            config('FILETYPE'));
    end
    ...
    if config('EXPORT_UNFILTERED_COMPOSITE')
        ...
        if config('PL_CHANNEL')
            ...
            gal8.*config('GAL8_BRIGHTEN')...
            + PL.*config('PL_BRIGHTEN'),...
    ...
    if config('EXPORT_COMPOSITE')
        ...
        if config('PL_CHANNEL')
            ...
            uint16(gal8Thresh.*config('GAL8_BRIGHTEN'))...
            + PLThresh.*config('PL_BRIGHTEN'),...
            nucThresh.*config('NUC_BRIGHTEN')...
            + NPThresh.*config('NP_BRIGHTEN')...
            + PLThresh.*config('PL_BRIGHTEN'));
