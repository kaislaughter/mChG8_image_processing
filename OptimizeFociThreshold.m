function [gal8ThreshOptimal] = OptimizeFociThreshold(config, channel,...
    baseDir)
...
%   Detailed explanation goes here
    ...
    for i = 1:numPC
        ...
        fociChannel = series1{channel, 1};
        PCData{i, 1} = imtophat(fociChannel, config('SE_GAL8_TH'));
    ...
    for i = 1:numNC
        ...
        series1 = data{1, 1};
        % Store the tophat-transformed gal8 channel.
        fociChannel = series1{fociChannel, 1};
        NCData{i, 1} = imtophat(fociChannel, config('SE_GAL8_TH'));
        % Store the number of nuclei.
        nuc = series1{config('NUC_CHANNEL'), 1};
        result = identifyNuclei(nuc, config('SE_NUC_OP'));
        NCData{i, 2} = result{1};
